<template>
    <div class="space-y-4">
        <div class="flex items-center justify-between">
            <h1 class="text-xl font-semibold text-slate-800 uppercase">Tipos de Horas Complementarias</h1>
            ...
            <template #cell-opciones="{ row }">
                <div class="flex items-center justify-center gap-2">
                    ...
                    <button type="button" class="border p-1.5 rounded-md transition"
                        ...
                        @click="row.collegeId !== null && confirmDelete(row)">
                ...
                <div v-if="modalOpen" class="fixed inset-0 z-50 flex items-center justify-center">
                    ...
                    <div class="relative bg-white rounded-xl shadow-2xl w-full max-w-2xl mx-4 p-6 space-y-4">
                        <h2 class="text-sm font-bold uppercase tracking-widest text-slate-700">
                            {{ editing ? 'Editar Tipo de Hora Complementaria' : 'Nuevo Tipo de Hora Complementaria' }}
                        </h2>
                ...
                <div v-if="deleteTarget" class="fixed inset-0 z-50 flex items-center justify-center">
                    ...
                    <div class="relative bg-white rounded-xl shadow-2xl w-full max-w-sm mx-4 p-6 space-y-4">
                        ...
                        <p class="text-sm text-slate-600">
                            ¿Eliminar <span class="font-semibold">{{ deleteTarget.name }}</span>? Esta acción no se puede deshacer.
                        </p>
...
const { rows, loading, pagination, handleChange, fetchData } = useDataTableFetch<ComplementaryHourType>({
    endpoint: API.SCA_API.complementaryHourTypes.paginate,
})
...
async function submitForm() {
    ...
    try {
        ...
        if (editing.value) {
            await api.put(API.SCA_API.complementaryHourTypes.update(editing.value.id), payload)
        } else {
            await api.post(API.SCA_API.complementaryHourTypes.create, payload)
        }
        closeModal()
        fetchData()
    } catch (e: any) {
...
async function doDelete() {
    ...
    deleting.value = true
    try {
        await api.delete(API.SCA_API.complementaryHourTypes.delete(deleteTarget.value.id))
