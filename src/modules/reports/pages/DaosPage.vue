<template>
    <div class="space-y-4">
        <div class="flex items-center justify-between">
            <h1 class="text-xl font-semibold text-slate-800 uppercase">Objetos de Acceso a Datos</h1>
            <button
                type="button"
                class="flex items-center gap-2 px-4 py-2 text-sm rounded-lg bg-blue-600 text-white hover:bg-blue-700"
                @click="router.push({ name: 'reports.daos.create' })"
            >
        ...
        >
            <template #cell-type="{ row }">
                <span class="px-2 py-1 text-xs font-semibold rounded-full" :class="typeClass(row.type)">
                    {{ typeLabel(row.type) }}
                </span>
            </template>
            ...
            <template #cell-opciones="{ row }">
                <div class="flex items-center justify-center gap-2">
                    <button aria-label="Ver" type="button" class="border p-1.5 rounded-md text-slate-500 hover:text-blue-600 hover:bg-blue-50 transition" title="Ver"
                        @click="router.push({ name: 'reports.daos.show', params: { id: row.id } })">
                    ...
                    <button aria-label="Editar" type="button" class="border p-1.5 rounded-md text-slate-500 hover:text-amber-600 hover:bg-amber-50 transition" title="Editar"
                        @click="router.push({ name: 'reports.daos.edit', params: { id: row.id } })">
                    ...
                    <button aria-label="Eliminar" type="button" class="border p-1.5 rounded-md text-slate-500 hover:text-red-600 hover:bg-red-50 transition" title="Eliminar"
                        @click="router.push({ name: 'reports.daos.delete', params: { id: row.id } })">
...
const columns: DataTableColumn<DataAccessObject>[] = [
...
const { rows, loading, pagination, handleChange, fetchData } = useDataTableFetch<DataAccessObject>({
    endpoint: API.REPORTS_API.daos.list,
})
