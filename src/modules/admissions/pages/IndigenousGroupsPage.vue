<template>
    <div class="space-y-4">
        <div class="flex items-center justify-between">
            <h1 class="text-xl font-semibold text-slate-800 uppercase">Grupos Indígenas</h1>
            ...
            <Transition enter-active-class="transition-opacity duration-150" leave-active-class="transition-opacity duration-150" enter-from-class="opacity-0" leave-to-class="opacity-0">
                <div v-if="modalOpen" class="fixed inset-0 z-50 flex items-center justify-center">
                    ...
                    <div class="relative bg-white rounded-xl shadow-2xl w-full max-w-sm mx-4 p-6 space-y-4">
                        <h2 class="text-sm font-bold uppercase tracking-widest text-slate-700">{{ editing ? 'Editar Grupo Indígena' : 'Nuevo/a Grupo Indígena' }}</h2>
                        <div>
...
const { rows, loading, pagination, handleChange, fetchData } = useDataTableFetch<CatalogItem>({ endpoint: API.ADMISSIONS_API.indigenousGroups.paginate })
...
async function submitForm() {
    ...
    try {
        if (editing.value) { await api.put(API.ADMISSIONS_API.indigenousGroups.update(editing.value.id), { name: form.value.name }) }
        else { await api.post(API.ADMISSIONS_API.indigenousGroups.create, { name: form.value.name }) }
...
async function doDelete() {
    ...
    deleting.value = true
    try { await api.delete(API.ADMISSIONS_API.indigenousGroups.delete(deleteTarget.value.id)); deleteTarget.value = null; fetchData() }
