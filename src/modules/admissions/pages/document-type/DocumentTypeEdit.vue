<template>
    <div class="max-w-xl space-y-6">
        <div class="flex items-center justify-between">
            <h1 class="text-xl font-semibold text-slate-800">EDITAR TIPO DE DOCUMENTO</h1>
            <button class="px-3 py-2 text-sm border rounded-lg hover:bg-slate-100" @click="router.back()">VOLVER</button>
        </div>

        <div v-if="loadingData" class="text-sm text-slate-400 py-8 text-center">Cargando...</div>

        <div v-else class="bg-white border rounded-xl shadow-sm p-6 space-y-5">
            <FormInput label="NOMBRE" v-model="form.name" />
            <FormInput label="NOMBRE CORTO" v-model="form.short_name" />
            <FormInput label="CÓDIGO" v-model="form.code" :maxlength="20" />
            <FormInput label="DESCRIPCIÓN" v-model="form.description" />

            <!-- Formatos aceptados -->
            <div class="space-y-2">
                <label class="block text-xs font-semibold text-slate-500 uppercase tracking-widest">FORMATOS PERMITIDOS</label>
                <div class="flex flex-wrap gap-2">
                    <label
                        v-for="fmt in FORMAT_OPTIONS"
                        :key="fmt.value"
                        class="flex items-center gap-1.5 text-sm cursor-pointer"
                    >
                        <input
                            type="checkbox"
                            :value="fmt.value"
                            v-model="form.accepts_formats"
                            class="rounded border-slate-300"
                        />
                        {{ fmt.label }}
                    </label>
                </div>
                <p class="text-xs text-slate-400">Deja vacío para permitir todos los formatos.</p>
            </div>

            <!-- Tamaño máximo con selector de unidad -->
            <div class="space-y-1.5">
                <label class="block text-[10px] font-black text-slate-400 uppercase tracking-wider">TAMAÑO MÁXIMO</label>
                <div class="flex gap-2">
                    <input
                        type="number"
                        min="1"
                        v-model="sizeValue"
                        placeholder="Ej. 5"
                        class="w-full border-2 rounded-xl px-4 py-2.5 text-sm font-bold border-slate-200 focus:border-blue-500 focus:ring-4 focus:ring-blue-50 outline-none"
                    />
                    <select
                        v-model="sizeUnit"
                        class="border-2 rounded-xl px-3 py-2.5 text-sm font-bold border-slate-200 focus:border-blue-500 outline-none bg-white"
                    >
                        <option value="KB">KB</option>
                        <option value="MB">MB</option>
                        <option value="GB">GB</option>
                    </select>
                </div>
            </div>

            <FormSwitch label="ACTIVO" v-model="form.status" />

            <p v-if="error" class="text-sm text-red-600">{{ error }}</p>

            <div class="flex justify-end gap-2 pt-4 border-t">
                <button class="px-4 py-2 text-sm border rounded-lg" @click="router.back()">CANCELAR</button>
                <button
                    class="px-4 py-2 text-sm rounded-lg bg-blue-600 text-white hover:bg-blue-700 disabled:opacity-60"
                    :disabled="submitting"
                    @click="submit"
                >
                    GUARDAR
                </button>
            </div>
        </div>
    </div>
</template>

<script setup lang="ts">
import { ref, computed, onMounted } from 'vue'
import { useRoute, useRouter } from 'vue-router'
import { api } from '@/shared/services/api'
import { API } from '@/shared/api'
import FormInput from '@/app/components/ui/form/FormInput.vue'
import FormSwitch from '@/app/components/ui/form/FormSwitch.vue'
import { FORMAT_OPTIONS } from '@/modules/admissions/types/document-type.type'

const route = useRoute()
const router = useRouter()
const loadingData = ref(true)
const submitting = ref(false)
const error = ref<string | null>(null)

const sizeValue = ref<number | null>(null)
const sizeUnit = ref<'KB' | 'MB' | 'GB'>('MB')

const SIZE_MULTIPLIERS = { KB: 1, MB: 1024, GB: 1048576 }

const maxSizeKb = computed(() =>
    sizeValue.value ? Math.round(sizeValue.value * SIZE_MULTIPLIERS[sizeUnit.value]) : null
)

function detectUnit(kb: number): { value: number; unit: 'KB' | 'MB' | 'GB' } {
    if (kb >= 1048576 && kb % 1048576 === 0) return { value: kb / 1048576, unit: 'GB' }
    if (kb >= 1024 && kb % 1024 === 0) return { value: kb / 1024, unit: 'MB' }
    return { value: kb, unit: 'KB' }
}

const form = ref({
    name: '',
    short_name: '',
    code: '',
    description: '',
    accepts_formats: [] as string[],
    status: true,
})

async function fetchData() {
    loadingData.value = true
    try {
        const { data } = await api.get(API.ADMISSIONS_API.documentTypes.byId(route.params.id as string))
        form.value = {
            name: data.name ?? '',
            short_name: data.shortName ?? '',
            code: data.code ?? '',
            description: data.description ?? '',
            accepts_formats: data.acceptsFormats ?? [],
            status: data.status ?? true,
        }
        if (data.maxSizeKb) {
            const detected = detectUnit(data.maxSizeKb)
            sizeValue.value = detected.value
            sizeUnit.value = detected.unit
        }
    } finally {
        loadingData.value = false
    }
}

async function submit() {
    error.value = null
    submitting.value = true
    try {
        const payload = {
            ...form.value,
            accepts_formats: form.value.accepts_formats.length ? form.value.accepts_formats : null,
            max_size_kb: maxSizeKb.value,
        }
        await api.put(API.ADMISSIONS_API.documentTypes.update(route.params.id as string), payload)
        router.push({ name: 'admissions.document-types.show', params: { id: route.params.id } })
    } catch (e: any) {
        error.value = e?.response?.data?.message ?? 'Error al guardar.'
    } finally {
        submitting.value = false
    }
}

onMounted(fetchData)
</script>
