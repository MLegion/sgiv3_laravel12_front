<template>
    <div class="max-w-5xl space-y-6 relative">
        ...
        <div class="bg-white border rounded-xl shadow-sm relative overflow-hidden">
            ...
            <div v-if="subject?.id || loading" class="p-6 space-y-8">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-8">
                    <div class="space-y-6">
                        ...
                        <div class="space-y-6">
                            <InfoItem label="INSTITUTO / FACULTAD" :value="subject.college?.name" :loading="loading" />
                        </div>
                    </div>
                    ...
                    <div class="space-y-6">
                        ...
                        <div class="grid grid-cols-3 gap-3">
                            <InfoItem label="HT" :value="subject.ht?.toString()" :loading="loading" />
                            <InfoItem label="HP" :value="subject.hp?.toString()" :loading="loading" />
                            <InfoItem label="CRÉDITOS" :value="subject.credits?.toString()" :loading="loading" />
                        </div>
                    </div>
                </div>
                ...
                <section class="pt-8 border-t">
                    ...
                    <div v-else class="bg-slate-50 border border-dashed rounded-lg p-4 flex items-center gap-3 text-slate-500">
                        ...
                        <span class="text-xs font-medium uppercase tracking-tighter">
                            Esta materia ya se encuentra en estado <b class="text-slate-700">{{ statusLabel }}</b> y no admite más acciones de validación.
                        </span>
                    </div>
                </section>
            </div>
            ...
            <div v-else-if="!loading" class="p-20 text-center">
                <p class="text-slate-400 text-sm font-medium">No se pudo cargar la información de la materia.</p>
...
const subject = ref<SubjectType>({} as SubjectType)

async function fetchData() {
    loading.value = true
    try {
        const { data } = await api.get(
            API.SUPERADMIN_API.subjects.byId(route.params.id as string)
        )
        subject.value = data
    } finally {
...
const canValidate = computed(() =>
    ![ApprovalStatusEnum.APPROVED, ApprovalStatusEnum.REJECTED]
        .includes(subject.value.approvalStatus as ApprovalStatusEnum)
)

const canEdit = computed(() =>
    subject.value.approvalStatus !== ApprovalStatusEnum.APPROVED
)

const statusLabel = computed(() => {
    switch (subject.value.approvalStatus) {
...
async function approve() {
    loading.value = true
    try {
        await api.post(API.SUPERADMIN_API.subjects.approve(subject.value.id))
        await fetchData()
        router.push({
...
async function reject() {
    loading.value = true
    try {
        await api.post(API.SUPERADMIN_API.subjects.reject(subject.value.id))
        await fetchData()
        router.push({
            name: 'superadmin.subjects',
        })
