...
const optionalGroupId = route.params.id as string

// Estados de Datos
const optionalGroup = ref<any>({})
...
const fetchData = async () => {
    loading.value = true
    try {
        const specRes = await api.get(API.SUPERADMIN_API.optionalGroups.byId(optionalGroupId))
        optionalGroup.value = specRes.data
        const planRes = await api.get(API.SUPERADMIN_API.curriculum.list(optionalGroup.value.studyPlanId) + `?optional_group_id=${optionalGroupId}`)
...
const isOptionalGroupItem = (item: any) => {
    if (!item) return false
    return String(item.optionalGroupId) === String(optionalGroupId) ||
        String(item.subject?.optionalGroupId) === String(optionalGroupId)
}
const isSpecialtyItem = isOptionalGroupItem
...
const onMoveItem = (payload: any) => {
    if (!isOptionalGroupItem(payload)) return
...
const handleAddSubject = async () => {
    ...
    submitting.value = true
    try {
        await api.post(API.SUPERADMIN_API.curriculum.create(optionalGroup.value.studyPlanId), {
            optional_group_id: optionalGroupId,
...
<template>
    <div class="space-y-6 max-w-[1600px] mx-auto p-4 font-sans bg-slate-50 min-h-screen">
        ...
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center bg-white p-5 rounded-[2rem] border border-slate-200 shadow-sm gap-4">
            <div v-if="optionalGroup.id" class="flex items-center gap-4">
            ...
            <div class="bg-white rounded-[2rem] shadow-2xl w-full max-w-md border border-slate-100 overflow-hidden">
                ...
                <div class="p-8 space-y-6">
                    <FormRemoteSelect
                        ...
                        :endpoint="API.SUPERADMIN_API.optionalGroups.listSubjescts(optionalGroupId)"
                        :params="{ optional_group_id: optionalGroupId, exclude_assigned_study_plan: true }"
                        item-label="name"
                        item-value="id"
