...
const advisingApi: ApiModule = {
    ...
    api: {
        ...
        addDrop: {
            myActivePeriod: apiUrl('/advising/add-drop/my-active-period'),
            myCurrent:      apiUrl('/advising/add-drop/my-current'),
            create:         apiUrl('/advising/add-drop/requests'),
            pending:        apiUrl('/advising/add-drop/requests/pending'),
            byId:           (id: string | number) => apiUrl(`/advising/add-drop/requests/${id}`),
            addItem:        (id: string | number) => apiUrl(`/advising/add-drop/requests/${id}/items/add`),
            dropItem:       (id: string | number) => apiUrl(`/advising/add-drop/requests/${id}/items/drop`),
            removeItem:     (id: string | number) => apiUrl(`/advising/add-drop/requests/${id}/items`),
            submit:         (id: string | number) => apiUrl(`/advising/add-drop/requests/${id}/submit`),
            cancel:         (id: string | number) => apiUrl(`/advising/add-drop/requests/${id}/cancel`),
            approve:        (id: string | number) => apiUrl(`/advising/add-drop/requests/${id}/approve`),
            reject:         (id: string | number) => apiUrl(`/advising/add-drop/requests/${id}/reject`),
        },
