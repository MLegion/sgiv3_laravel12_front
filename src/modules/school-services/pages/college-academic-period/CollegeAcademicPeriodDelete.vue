<template>
    <div class="max-w-md space-y-6">
        <h1 class="text-xl font-semibold text-slate-800">ELIMINAR - PERIODO INSTITUCIONAL</h1>

        <div class="bg-white border rounded-xl shadow-sm p-6 space-y-4">
            <p class="text-sm text-slate-600">
                ¿ESTÁS SEGURO DE ELIMINAR EL PERIODO <strong>{{ name }}</strong> DE TU INSTITUCIÓN?
            </p>
...
async function fetchName() {
    ...
    name.value = ap ? `${ap.name} (${ap.shortName})` : `Periodo #${data.id}`
}
